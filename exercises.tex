\documentclass[12pt]{exam}  % exam document class - MIT Mathematics
\usepackage[utf8]{inputenc}

\input{ex_preamble}

%%%%%%%%% Ausgabe der Lösungen %%%%%%%%%

%\noprintanswers
\printanswers

%%%%%%%%% Rahmen um Lösung %%%%%%%%%
\unframedsolutions
%\framedsolutions

%römische Zahlen in Fließtext definieren
%\newcommand{\rom}[1]{$\underline{\overline{\text{#1}}}$}

\begin{document}
		\input{ex_layout}
		\input{tikz_settings}
	\begin{coverpages}
		\huge\centering Übungsaufgaben Bodendynamik - SS \the\year 
		\normalsize
	\end{coverpages}
	
	\begin{questions}
	
		\section{Einleitung}
		%\fullwidth{\Large \textbf{Essay questions}}
		\input{ex01_introduction}
		
		\newpage
		\section{Grundlagen}
		   \subsection{Einfreiheitsgradschwingungen}
		   \input{ex02_harmonic_oscillator}
		   \subsection{Frequenzanalyse}
		   \input{ex03_frequency_analysis}
		   \subsection{Mechanisches Bodenverhalten}
		   \input{ex04_soil_models}
		
		\newpage
		\section{Wellenausbreitung im Untergrund}
		   \subsection{Dehnstabsmodell (1D)}
		   \input{ex05_wave1d}
		   \subsection{Elastodynamik (3D)}
		   \input{ex06_wave3d}
		
		\newpage
		\section{Praktische Anwendung}
		   \subsection{Kennwertermittlung}
		   \input{ex07_measurement}
		   \subsection{Konstruktionskriterien}
		   \input{ex08_design}
	
	\end{questions}

\end{document}
